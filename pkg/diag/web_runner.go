...
func IsValidWebMode(m WebMode) bool {
	switch m {
	case WebModeAll, WebModePublicIP, WebModeDNS, WebModeHTTP, WebModePort:
		return true
	}
	return false
}
...
//   - WebModeHTTP, WebModePort: this runner is a no-op; other runners handle those modes.
func (r *WebRunner) Run(ctx context.Context, req Request) error {
	...
	if !runPublicIP && !runDNS {
		return nil // http or port mode — not this runner's responsibility
	}
